...
- [`AmdTscSync`](https://github.com/naveenkrdy/AmdTscSync){target="_blank"} (10.13+)
  ...
  Synchronises the TimeStamp Counter (TSC). Generally only useful for AMD APUs (usually laptops).

  In its `Info.plist`, `IOCPUNumber`'s value should be `0` on macOS 12.0 and newer, on any older version it should be the number of threads in your CPU minus one (e.g. 8 thread CPU -> 7).
