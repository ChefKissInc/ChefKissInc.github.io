...
Afterwards, copy `Sample.plist` as `config.plist` inside of `EFI/OC`. That's OpenCore's configuration which will be edited for our system further down the line.
