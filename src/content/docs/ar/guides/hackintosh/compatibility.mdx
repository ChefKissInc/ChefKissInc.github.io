...
<Aside type="caution">
تدعم التصحيحات نظام التشغيل macOS 10.13 وأحدث فقط، وتتطلب الإصدارات الأقدم نواة مخصصة، وهو أمر غير مدعوم في هذا الدليل.
</Aside>
...
<Aside type="danger">
هناك بعض الأشياء في نظام macOS لا تعمل بشكل صحيح مع وحدة المعالجة المركزية AMD, انظر [أدناه](#amd-cpu-incompatibilities).
</Aside>
...
- `SSE4.2` مطلوب لـ `macOS 10.14`+ وأحدث برامج تشغيل AMD GPU. تجنب `SSE4.1` - وحدات المعالجة المركزية فقط إن أمكن. قد يعمل مع بعض الاختراقات؛ انظر [جمع الملفات > Kexts](/guides/hackintosh/gathering-files/kexts/)

### عدم توافقات وحدة المعالجة المركزية AMD
...
- Krisp
- Logic Pro Waves المكونات الإضافية
- Adobe برامج
- Discord

#### تطبيقات 32 بت

لا تدعم تصحيحات `AMD Vanilla` التطبيقات ذات 32 بت، حتى في `WINE`/`CrossOver`. ومع ذلك، فهي تعمل بطريقة ما على Threadripper (TRX40).

## دعم وحدة معالجة الرسومات

دعم وحدة معالجة الرسوميات معقد.
...
- لا يتم دعم وحدات معالجة الرسوميات RDNA 3، كما لا يتم دعم Navi 24 (حتى الآن). Navi 22 مدعوم بواسطة [`NootRX`](/applehax/nootrx/)
- [Raven](https://www.techpowerup.com/gpu-specs/amd-raven.g816) و [Renoir](https://www.techpowerup.com/gpu-specs/amd-renoir.g1058) AMD iGPUs مدعومة عبر [`NootedRed`](/applehax/nootedred/).
- أجيال iGPU الأخرى (GCN \<5، RDNA) **غير** مدعومة على الإطلاق (حتى الآن).
- [Baffin](https://www.techpowerup.com/gpu-specs/amd-baffin.g796) يتم دعم وحدات معالجة الرسومات AMD في أحدث إصدار من macOS.
- [Ellesmere](https://www.techpowerup.com/gpu-specs/amd-ellesmere.g795) يتم دعم وحدات معالجة الرسومات AMD في أحدث إصدار من macOS.
- [Lexa](https://www.techpowerup.com/gpu-specs/amd-lexa.g806) يتم دعم وحدات معالجة الرسوميات AMD من خلال خداع معرف الجهاز لمتغير Baffin المعني.
- وحدات Polaris 20 AMD dGPU غير مدعومة (حتى الآن).

<Aside type="danger">
مستخدمو **MSI Navi**: [المثبت لا يعمل مع 5700XT #901](https://github.com/acidanthera/bugtracker/issues/901), **تم حلها منذ `macOS 11`**.
</Aside>
