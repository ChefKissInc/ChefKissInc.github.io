...
- `SSE4.2` is required for `macOS 10.14`+ and newer AMD GPU drivers. Avoid `SSE4.1`-only CPUs if possible. May work with some hacks; see [Gathering Files > Kexts](/en/guides/hackintosh/gathering-files/kexts)
...
- RDNA 3 GPUs are unsupported, Navi 24 is also not supported (yet). Navi 22 is supported by [`NootRX`](/en/applehax/nootrx)
- [Raven](https://www.techpowerup.com/gpu-specs/amd-raven.g816) and [Renoir](https://www.techpowerup.com/gpu-specs/amd-renoir.g1058) AMD iGPUs are supported via [`NootedRed`](/en/applehax/nootedred).
...
With that out of the way, let's start with [gathering the required files](/en/guides/hackintosh/gathering-files).
